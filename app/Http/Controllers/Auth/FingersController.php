...
class FingersController extends Controller {
	...
	function getEdit($message = null)
	{
		...
		$this->layout->page_title 	= 'Finger Set Up';

		$this->layout->content 		= view('admin.pages.fingers.form', compact('client', 'secret'));

		if(!is_null($message))
		{
			$this->layout->content 	= view('admin.pages.fingers.form', compact('client', 'secret'))->with('alert_success', $message);
		}
