...
<div class="hbox-column col-md-2" id="sidebar_left">
	<ul class="nav nav-pills nav-stacked">
		...
		<li @if(isset($calendars)) class="active" @endif><a href="{{route('hr.organisation.calendars.index', ['page' => 1, 'org_id' => $data['id']])}}">@if(!$data['has_calendars']) <i class="fa fa-exclamation pull-right mt-5 text-warning"></i> @endif Kalender  </a></li>
		<li @if(isset($workleaves)) class="active" @endif><a href="{{route('hr.organisation.workleaves.index', ['page' => 1, 'org_id' => $data['id']])}}">@if(!$data['has_workleaves']) <i class="fa fa-exclamation pull-right mt-5 text-warning"></i> @endif Cuti  </a></li>
